...
proc lwip_elite_hw_drc {libhandle emac} {
	...
	set tx_full_csum [xget_value $libhandle "PARAMETER" "tcp_ip_tx_checksum_offload"]
 	...
 	set igmp_val [xget_value $libhandle "PARAMETER" "igmp_options"]
	...
	if {$rx_csum} {
 	...
 	if {$igmp_val == true} {
 		error "IGMP is not supported for emaclite" "" "MDT_ERROR"
 	}
}
	...
	if { [string compare -nocase "SOCKET_API" $api_mode] == 0} {
		...
		if { [string compare -nocase "xilkernel" $os_name] != 0} {
			if { [string compare -nocase "freertos_zynq" $os_name] != 0} {
				error "lwIP with Sockets requires \"xilkernel or freertos\" OS" "" "mdt_error"
			}
		}
	}
}
...
proc lwip_drc {libhandle} {
	...
	if { [llength $emac_periphs_list] == 0 } {
		...
		error "No Ethernet MAC cores are addressable from processor $cpuname. \
			lwIP requires atleast one EMAC (xps_ethernetlite | xps_ll_temac | axi_ethernet | axi_ethernetlite | ps7_ethernet) core \
...
proc generate_lwip_opts {libhandle} {
	...
	set jumbo_frames [xget_value $libhandle "PARAMETER" "temac_use_jumbo_frames"]
	if {$jumbo_frames} {
	...
	set dhcp_does_arp_check [expr [xget_value $libhandle "PARAMETER" "dhcp_does_arp_check"] == true]
	...
	set udp_debug		[expr [xget_value $libhandle "PARAMETER" "udp_debug"] == true]
	set icmp_debug		[expr [xget_value $libhandle "PARAMETER" "icmp_debug"] == true]
	set igmp_debug		[expr [xget_value $libhandle "PARAMETER" "igmp_debug"] == true]
...
proc update_ps7_ethernet_topology {emac processor topologyvar} {
	upvar $topologyvar topology
	set topology(emac_baseaddr) [xget_hw_parameter_value $emac "C_S_AXI_BASEADDR"]
	...
	set topology(scugic_emac_intr) "0x0"
	set ethernet_instance [xget_hw_parameter_value $emac "INSTANCE"]
...
proc generate_adapterconfig_include {libhandle} {
	...
	if {$have_ps7_ethernet == 1} {
		set emacnum [xget_value $libhandle "PARAMETER" "emac_number"]
